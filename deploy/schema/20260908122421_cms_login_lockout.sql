...
begin
  ...
  -- Keep the guard table bounded without exposing or relying on client identity.
  ...
  if cred.username is not null and normalized_hash <> '' then
    ...
    -- Perform a bcrypt operation for unknown usernames to reduce timing differences.
  ...
  -- A correct password clears prior failures for this username.
  delete from public.cms_admin_login_guards
  where username_hash = username_key;
...
-- Changing a password from an already authenticated CMS session also clears any
-- accumulated failed-login state for that account.
