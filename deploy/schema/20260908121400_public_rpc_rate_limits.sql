...
revoke all on function public.consume_public_rate_limit(text,text,integer,interval,text) from public, anon, authenticated;
...
begin
  ...
  perform public.consume_public_rate_limit(
    'tracking',
    upper(trim(coalesce(p_tracking_code, ''))) || ':' || right(public.normalize_digits(p_last4), 4),
    10,
    interval '15 minutes',
